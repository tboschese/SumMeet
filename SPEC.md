...
| **A16** ✅ | Home calendar view | Browse the history as a month calendar, not just a list | **Shipped (2026-08).** A List/Calendar toggle on the home page. The calendar marks each day with how many meetings it holds and a dot for the ones you took your own notes in; clicking a day lists them, linking through. A dedicated unpaginated `/api/meetings/calendar?from&to` endpoint returns the grid's window; the client fetches it as instants and buckets by **local** date, so a late-night meeting lands on the day you had it, not a UTC-shifted one. No new storage — a read over the meetings already there. |
