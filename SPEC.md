...
- ❌ **Real-time / streaming transcription** — record fully, then process (batch). Live transcription is later. *(Shipped post-MVP as A18 — the full recording is still uploaded at stop and remains the authority.)*
...
| **A10** | Folders & organization | Group meetings into user-defined folders; the list filters by folder | **New (2026-07).** The history is flat and grows without bound; pagination and search (shipped) help find a *known* meeting, folders give *structure* for recurring contexts — per client, per project, per team. Model it as a nullable `folderId` on `Meeting` (a meeting is in zero or one folder) plus a flat `Folder` table; the existing list filters gain a folder facet, and the trash stays orthogonal. No new infra — another SQLite table. The Markdown export (shipped) already files one summary into a real OS folder; this is the in-app counterpart. |
| **A11** | Email connect — Gmail & Outlook | Deliver a meeting's decision record to email; optionally read the calendar invite for context | **New (2026-07).** Specialises A4's "email delivery" into the two providers people actually use, via their APIs (Gmail API / Microsoft Graph) with OAuth. Two directions: *out* — send the summary as a formatted email or thread reply after the meeting; *in* — read the calendar invite to seed the meeting title and feed attendee names into the glossary (A6) so owners are spelled right. **OAuth tokens are secrets: server/app-side only, never shipped to a browser client (hard rule §7.2), stored centrally like the API keys (A0).** Local-first stays intact — this is an outbound integration the user opts into, not a dependency the pipeline needs. |
| **A12** ✅ | MCP server over the summaries | An MCP server exposing the meeting history so an assistant can query it | **Shipped.** The insights are already structured, Zod-validated records in SQLite; an MCP server turns them into a queryable tool surface with no extra UI. Fits local-first exactly: it runs against the **local** DB, read-only by default, and nothing leaves the machine unless the user points a cloud assistant at it. Natural tools: `search_meetings(query, dateRange)`, `get_meeting(id)`, `list_commitments(owner="You")`, `list_decisions`. Read-only first; mutations (retag, move to folder) later. This is A4's "consume value without opening the app" taken to its conclusion — the summaries become a knowledge base an agent can reason over ("what did I commit to this week?", "every decision about pricing"). |
| **A13** ✅ | Templates per meeting type | Named recipes of summary sections, one marked default | **Shipped (2026-08).** A4/A5 already let the user compose which sections a summary carries; templates make that composition *reusable and named* — a 1:1 wants commitments and next steps, a sales call wants risks and metrics, a standup wants only actions. Built-ins are seeded (`General`, `1:1`, `Sales call`, `Standup`, `Interview`, `Retro`) and read-only; the user duplicates one to edit, and picks which is the default applied to new recordings. Model it as a flat `Template` table (name + ordered `SectionKey[]`) plus a nullable `sections` on `Meeting` (this meeting's own recipe, else the default). No new infra — another SQLite table. The detail page can re-run the summary with any template; the previous version is kept via the existing insight versioning, so switching recipes is safe to undo. |
| **A14** ✅ | Enhance my notes | Expand the rough notes you jot during a call from the transcript | **Shipped (2026-08).** The user takes sparse notes in the widget while the meeting runs; afterwards each note line becomes an anchor and the LLM fills in what the transcript actually said about it, grounded in a verbatim `sourceQuote`. The user's line is preserved exactly (shown dark), the detail is the AI's addition (shown gray), and it never invents — a note the transcript doesn't cover gets an empty detail. Runs best-effort at the end of the pipeline whenever notes exist; also on demand from the detail page. Reuses the same `LlmProvider` seam and the notes column already on `Meeting`. |
| **A15** ✅ | My work dashboard | A standing cross-meeting view of the commitments you own and the decisions made | **Shipped (2026-08).** The insights already carry per-meeting action items (with `owner`, `dueDate`, `priority`) and decisions; A12 turned them into cross-meeting aggregates for the MCP, but nothing in the app surfaced them. This is that missing UI: a `/commitments` page answering "what did I agree to?" without opening each meeting or asking the assistant. Commitments bucket by due-date urgency (overdue / today / this week / later / no date — vague phrases like "next Friday" fall to no-date but keep their text), with a Mine/Everyone owner filter keyed on `isMine`; decisions show their rationale. Every row links back to its meeting and can reveal the grounding quote. Read-only, no LLM call — pure SQLite aggregation over each meeting's active insight version, so it costs nothing to open. |
| **A16** ✅ | Home calendar + day agenda | Browse the history Granola-style: a compact calendar over a day-grouped agenda | **Shipped (2026-08).** The home default is a small, always-visible month calendar (dots mark days that have meetings; amber = you took notes) over an agenda that groups meetings by day, newest first — Today / Yesterday / a spelled-out date. It shows a few days and reveals earlier ones on demand; picking a day in the calendar narrows the agenda to it. A search or filter switches the area to the flat results list (content search is server-side, so it can't run over the agenda's titles-only data); the trash keeps its own list. Backed by an unpaginated `/api/meetings/calendar?from&to` returning every live meeting's minimal fields (incl. `hasNotes`, `folderId`); the client sends the window as instants and buckets by **local** date, so a late-night meeting sits on the day you had it, not a UTC-shifted one. No new storage. |
| **A17** ✅ | Echo cancellation on the mic | Stop the mic re-capturing the meeting audio when recording on speakers | **Shipped (2026-08).** Builds on the speaker-detection warning: an actual fix. When enabled (a Settings toggle, default on — `echoCancellation`), the native recorder captures the mic through the OS Voice-Processing I/O (`AVAudioEngine.setVoiceProcessingEnabled`), which removes the meeting audio the mic hears acoustically; AGC is disabled so the per-channel energy the diarizer compares stays honest. Applies to the **default** mic (device selection and voice processing don't mix), forwards the processed mono mic (channel 0), and — crucially for hard rule #5 — falls back to plain `AVCaptureSession` capture on any failure, so it can never cost the mic. The flag rides the existing start path: web setting → `start_recording(aec)` → recorder `--aec`. Verified the AEC path selects and keeps the mic alive; echo removal itself is best verified in a real speaker meeting. |

| **A18** ✅ | Transcribe while recording | Short chunks transcribed during the meeting, finalized at stop | **Shipped (2026-08).** The MVP transcribed the whole recording in one pass *after* the meeting, so the wait at stop grew with the meeting — painful with the local Whisper (A3), which runs well below realtime. The recorder now mirrors what it captures into short chunks (45–75 s), ships each to `POST /api/meetings/:id/segment` while the call runs, and finalizes at stop with `POST /api/meetings/:id/finish`. **The full recording is still uploaded at stop and stays the authority**: the server only trusts the live transcript if it covers ≥85% of the recording, and otherwise transcribes the file whole — a recorder that drops chunks degrades to the old behaviour instead of shipping a holey transcript. Chunks **overlap** (each runs 3 s past its cut) and cuts prefer a quiet window, because a hard cut swallows the phrase straddling it — measured. Whisper's per-call language detection is pinned by the first chunk, so a short chunk can't flip the transcript's language mid-meeting. Verified on a 3-minute meeting: the streamed transcript is word-identical to the one-pass control apart from ordinary Whisper variation. Off-switch: the `liveTranscription` setting (default on). |
